  ...
  where
    ...
    rn_ty env (HsForAllTy { hst_fvf = fvf, hst_bndrs = tvs
      ...
      = bindLHsTyVarBndrs (rtke_ctxt env) WarnUnusedForalls
        ...
        do { (hs_body', fvs) <- rn_lty env hs_body
           ; return (HsForAllTy { hst_fvf = fvf, hst_xforall = noAnn
    ...
    rn_ty env (HsQualTy { hst_ctxt = L cx hs_ctxt
      ...
      = do { (hs_ctxt1', fvs1) <- mapFvRn (rn_top_constraint env) hs_ctxt1
           ...
           ; return (HsQualTy { hst_xqual = noAnn
...
rnHsTyKi env ty@(HsQualTy { hst_ctxt = lctxt, hst_body = tau })
  = do { checkPolyKinds env ty  -- See Note [QualTy in kinds]
       ...
       ; return (HsQualTy { hst_xqual = noAnn, hst_ctxt = ctxt'
                          , hst_body =  tau' }
                , fvs1 `plusFV` fvs2) }
...
rnHsTyKi env (HsFunTy _ ty1 ty2)
  = do { (ty1', fvs1) <- rnLHsTyKi env ty1
       ...
       ; res_ty <- mkHsOpTyRn (HsFunTy noAnn) funTyConName funTyFixity ty1' ty2'
...
rnHsTyKi env t@(HsKindSig x ty k)
...
-- on each name.
bindLRdrNames :: [ApiAnnName RdrName]
              -> ([Name] -> RnM (a, FreeVars))
              -> RnM (a, FreeVars)
...
bindHsQTyVars :: forall a b.
              ...
              -> [ApiAnnName RdrName] -- Kind variables from scope, no dups
              -> LHsQTyVars GhcPs
...
bindHsQTyVars doc mb_assoc body_kv_occs hsq_bndrs thing_inside
  = do { let hs_tv_bndrs = hsQTvExplicit hsq_bndrs
       ...
       ; let -- See Note [bindHsQTyVars examples] for what
             ...
             implicit_kvs = nubN $ filterFreeVarsToBind bndrs $
               bndr_kv_occs ++ body_kv_occs
             del          = deleteBys eqApiAnnNamed
...
newTyVarNameRn :: Maybe a -> ApiAnnName RdrName -> RnM Name
newTyVarNameRn mb_assoc (N loc rdr)
  = do { rdr_env <- getLocalRdrEnv
       ; case (mb_assoc, lookupLocalRdrEnv rdr_env rdr) of
           (Just _, Just n) -> return n
              -- Use the same Name as the parent class decl

           _                -> newLocalBndrRn (N loc rdr) }
{-
*********************************************************
*                                                       *
        ConDeclField
...
mkHsOpTyRn mk1 pp_op1 fix1 ty1 (L loc2 (HsFunTy x ty21 ty22))
  = mk_hs_op_ty mk1 pp_op1 fix1 ty1
                (HsFunTy x) funTyConName funTyFixity ty21 ty22 loc2
...
extractHsTyRdrTyVars ty
  = nubN (extractHsTyRdrTyVarsDups ty)
...
extractHsTyVarBndrsKVs tv_bndrs
  = nubN (extract_hs_tv_bndrs_kvs tv_bndrs)

-- Returns the free kind variables in a type family result signature, returning
-- variable occurrences in left-to-right order.
-- See Note [Ordering of implicit variables].
extractRdrKindSigVars :: LFamilyResultSig GhcPs -> [ApiAnnName RdrName]
...
extract_tv :: ApiAnnName RdrName
           -> [ApiAnnName RdrName] -> [ApiAnnName RdrName]
extract_tv tv acc =
