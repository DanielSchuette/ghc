...
newLocalBndrRn :: ApiAnnName RdrName -> RnM Name
-- Used for non-top-level binders.  These should
-- never be qualified.
newLocalBndrRn (N loc rdr_name)
...
newLocalBndrsRn :: [ApiAnnName RdrName] -> RnM [Name]
...
-------------------------------------
checkDupRdrNames :: [ApiAnnName RdrName] -> RnM ()
...
---------------------
checkShadowedRdrNames :: [ApiAnnName RdrName] -> RnM ()
checkShadowedRdrNames loc_rdr_names
  = do { envs <- getRdrEnvs
       ; checkShadowedOccs envs get_loc_occ filtered_rdrs }
  where
    filtered_rdrs = filterOut (isExact . unApiName) loc_rdr_names
                -- See Note [Binders in Template Haskell] in Convert
    get_loc_occ (N loc rdr) = (locA loc,rdrNameOcc rdr)
...
data HsDocContext
  ...
  | SpecInstSigCtx
  | DefaultDeclCtx
  | ForeignDeclCtx (ApiAnnName RdrName)
  | DerivDeclCtx
  | RuleCtx FastString
  | TyDataCtx (ApiAnnName RdrName)
  | TySynCtx (ApiAnnName RdrName)
  | TyFamilyCtx (ApiAnnName RdrName)
  | FamPatCtx (ApiAnnName RdrName)    -- The patterns of a type/data family instance
  | ConDeclCtx [ApiAnnName Name]
  | ClassDeclCtx (ApiAnnName RdrName)
  | ExprWithTySigCtx
  | TypBrCtx
  | HsTypeCtx
  | GHCiCtx
