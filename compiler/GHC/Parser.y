...
signature :: { Located HsModule }
       : maybedocheader 'signature' modid maybemodwarning maybeexports 'where' body
             {% fileSrcSpan >>= \ loc ->
                acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) ([mj AnnSignature $2, mj AnnWhere $6] ++ fst $7) cs)
                                            (Just $3) $5 (fst $ snd $7)
                              (snd $ snd $7) $4 $1) )}
...
module :: { Located HsModule }
       : maybedocheader 'module' modid maybemodwarning maybeexports 'where' body
             {% fileSrcSpan >>= \ loc ->
                acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) ([mj AnnModule $2, mj AnnWhere $6] ++ fst $7) cs)
                                           (Just $3) $5 (fst $ snd $7)
                              (snd $ snd $7) $4 $1) ) }
        | body2
                {% fileSrcSpan >>= \ loc ->
                   acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) (fst $1) cs) Nothing Nothing
...
maybemodwarning :: { Maybe (LocatedL WarningTxt) }
    : '{-# DEPRECATED' strings '#-}'
                      {% fmap Just $ amsrl (sLL $1 $> $ DeprecatedTxt (sL1 $1 (getDEPRECATED_PRAGs $1)) (snd $ unLoc $2))
                              (AnnList (mo $1) (mc $3) (fst $ unLoc $2))}
    | '{-# WARNING' strings '#-}'
                         {% fmap Just $ amsrl (sLL $1 $> $ WarningTxt (sL1 $1 (getWARNING_PRAGs $1)) (snd $ unLoc $2))
                                 (AnnList (mo $1) (mc $3) (fst $ unLoc $2))}
    |  {- empty -}                  { Nothing }
...
header  :: { Located HsModule }
        : maybedocheader 'module' modid maybemodwarning maybeexports 'where' header_body
                {% fileSrcSpan >>= \ loc ->
                   acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) [mj AnnModule $2,mj AnnWhere $6] cs)
                                              (Just $3) $5 $7 [] $4 $1
                          )) }
...
semis1  :: { [AddApiAnn] }
semis1  : semis1 ';'  { if isZeroWidthSpan (gl $2) then $1 else (mj AnnSemi $2 : $1) }
        | ';'         { mz AnnSemi $1 }

-- Zero or more semicolons
semis   :: { [AddApiAnn] }
semis   : semis ';'   { if isZeroWidthSpan (gl $2) then $1 else (mj AnnSemi $2 : $1) }
        | {- empty -} { [] }
...
maybe_src :: { (([AddApiAnn],SourceText),IsBootInterface) }
        : '{-# SOURCE' '#-}'        { (([mo $1,mc $2],getSOURCE_PRAGs $1)
                                      , IsBoot) }
        | {- empty -}               { (([],NoSourceText),NotBoot) }

maybe_safe :: { ([AddApiAnn],Bool) }
        : 'safe'                                { ([mj AnnSafe $1],True) }
        | {- empty -}                           { ([],False) }

maybe_pkg :: { ([AddApiAnn],Maybe StringLiteral) }
...
maybeas :: { ([AddApiAnn],Located (Maybe (Located ModuleName))) }
        : 'as' modid                           { ([mj AnnAs $1]
                                                 ,sLL $1 $> (Just $2)) }
        | {- empty -}                          { ([],noLoc Nothing) }
...
ty_decl :: { LTyClDecl GhcPs }
        ...
        | 'type' 'family' type opt_tyfam_kind_sig opt_injective_info
                ...
                -- Note the use of type for the head; this allows
                -- infix type constructors to be declared
                {% mkFamDecl (comb4 $1 (reLoc $3) $4 $5) (snd $ unLoc $6) $3
...
deriv_strategy_via :: { LDerivStrategy GhcPs }
  : 'via' ktype             {% acs (\cs -> sLLlA $1 $> (ViaStrategy (XViaStrategyPs (ApiAnn (glR $1) [mj AnnVia $1] cs)
                                                                           (mkLHsSigType $2)))) }
...
ty_fam_inst_eqn :: { LTyFamInstEqn GhcPs }
        ...
        | type '=' ktype
              ...
              {% mkTyFamInstEqn (comb2 (reLoc $1) (reLoc $>)) Nothing $1 $3 (mj AnnEqual $2:[]) }
              -- Note the use of type for the head; this allows
...
at_decl_cls :: { LHsDecl GhcPs }
        :  -- data family declarations, with optional 'family' keyword
          'data' opt_family type opt_datafam_kind_sig
                {% liftM mkTyClD (mkFamDecl (comb3 $1 (reLoc $3) $4) DataFamily $3
                                                  (snd $ unLoc $4) Nothing
        ...
        | 'type' type opt_at_kind_inj_sig
               {% liftM mkTyClD
                        (mkFamDecl (comb3 $1 (reLoc $2) $3) OpenTypeFamily $2
                                   (fst . snd $ unLoc $3)
                                   (snd . snd $ unLoc $3)
                         (mj AnnType $1:(fst $ unLoc $3)) )}
        | 'type' 'family' type opt_at_kind_inj_sig
               {% liftM mkTyClD
                        (mkFamDecl (comb3 $1 (reLoc $3) $4) OpenTypeFamily $3
                                   (fst . snd $ unLoc $4)
                                   (snd . snd $ unLoc $4)
                         (mj AnnType $1:mj AnnFamily $2:(fst $ unLoc $4)))}
...
tycl_hdr :: { Located (Maybe (LHsContext GhcPs), LHsType GhcPs) }
        -- : context '=>' type         {% addAnnotation (glAR $1) (toUnicodeAnn AnnDarrow $2) (glR $2)
        --                                >> (return (sLLAA $1 $> (Just $1, $3)))
        --                             }
        -- AZ: working here
        : context '=>' type         {% do { let { (L (SrcSpanAnn (ApiAnn r (AnnContext _ o c) cs) l) ctxt) = $1 }
                                          ; let { u = if (isUnicode $2) then UnicodeSyntax else NormalSyntax }
                                          ; let { ann = SrcSpanAnn (ApiAnn r (AnnContext (Just (u, glR $2)) o c) cs) l }
                                          ; return (sLLAA $1 $> (Just (L ann ctxt), $3)) }
                                    }
        | type                      { sL1A $1 (Nothing, $1) }

tycl_hdr_inst :: { Located ([AddApiAnn],(Maybe (LHsContext GhcPs), Maybe [LHsTyVarBndr () GhcPs], LHsType GhcPs)) }
        : 'forall' tv_bndrs '.' context '=>' type   {% hintExplicitForall $1
                                                       >> fromSpecTyVarBndrs $2
                                                         >>= \tvbs -> (addAnnotation (glAR $4) (toUnicodeAnn AnnDarrow $5) (glR $5)
                                                             >> return (sLL $1 (reLoc $>) ([mu AnnForall $1, mj AnnDot $3]
                                                                                  , (Just $4, Just tvbs, $6)))
                                                          )
                                                    }
        ...
        | context '=>' type         {% addAnnotation (glAR $1) (toUnicodeAnn AnnDarrow $2) (glR $2)
                                       >> (return (sLLAA $1 $>([], (Just $1, Nothing, $3))))
                                    }
        | type                      { sL1A $1 ([], (Nothing, Nothing, $1)) }


capi_ctype :: { Maybe (LocatedA CType) }
...
pattern_synonym_lhs :: { (LocatedN RdrName, HsPatSynDetails (LocatedN RdrName), [AddApiAnn]) }
...
cvars1 :: { [RecordPatSynField (LocatedN RdrName)] }
                    ...
                    {% runECP_P $2 >>= \ $2 ->
                       do { v <- checkValSigLhs $2
                          ; let err = text "in default signature" <> colon <+>
                                      quotes (ppr $2)
                          ; acsA (\cs -> sLL $1 (reLoc $>) $ SigD noExtField $ ClassOpSig (ApiAnn (glR $1) [mj AnnDefault $1,mu AnnDcolon $3] cs) True [v] $ mkLHsSigType $4) }}
...
--
binds   ::  { Located ([AddApiAnn],Located (HsLocalBinds GhcPs)) }
...
wherebinds :: { Located ([AddApiAnn],Located (HsLocalBinds GhcPs)) }
                                                -- May have implicit parameters
                                                -- No type declarations
...
opt_sig :: { ([AddApiAnn], Maybe (LHsType GhcPs)) }
        : {- empty -}                   { ([],Nothing) }
...
forall_vis_flag :: { (AddApiAnn, ForallVisFlag) }
...
ktypedoc :: { LHsType GhcPs }
         : ctypedoc            { $1 }
         | ctypedoc '::' kind  {% acsA (\cs -> sLLAA $1 $> $ HsKindSig (ApiAnn (glAR $1) [mu AnnDcolon $2] cs) $1 $3) }
                                        ...
                                        {% let (fv_ann, fv_flag) = $3 in
                                           ...
                                           acsA (\cs -> sLL $1 (reLoc $>) $
                                                HsForAllTy { hst_fvf = fv_flag
                                                           , hst_bndrs = $2
                                                           , hst_xforall = ApiAnn (glR $1) [mu AnnForall $1,fv_ann] cs
                                                           , hst_body = $4 }) }
        | context '=>' ctype          {% acsA (\cs -> (sLLAA $1 $> $
                                            HsQualTy { hst_ctxt = $1
                                         ...
                                         {% let (fv_ann, fv_flag) = $3 in
                                            ...
                                            acsA (\cs -> sLL $1 (reLoc $>) $
                                                 HsForAllTy { hst_fvf = fv_flag
                                                            ...
                                                            , hst_xforall = ApiAnn (glR $1) [mu AnnForall $1,fv_ann] cs
                                                            , hst_body = $4 }) }
        | context '=>' ctypedoc       {% acsA (\cs -> (sLLAA $1 $> $
                                            HsQualTy { hst_ctxt = $1
                                                     , hst_xqual = ApiAnn (glAR $1) [mu AnnDarrow $2] cs
                                                     , hst_body = $3 }) )}
        | ipvar '::' type             {% acsA (\cs -> sLL $1 (reLoc $>) (HsIParamTy (ApiAnn (glR $1) [mu AnnDcolon $2] cs) $1 $3)) }
...
context :: { LHsContext GhcPs }
        :  btype                        {% do { ctx <- checkContext $1
                                                ; if null (unLoc ctx)
                                                   then addAnnotationS (glA $1) AnnUnit (glA $1)
                                                   else return ()
                                                ; return ctx
                                                } }
...
type :: { LHsType GhcPs }
        ...
        | btype '->' ctype             {% amsA $1 [mu AnnRarrow $2] -- See note [GADT decl discards annotations]
                                       >> acsA (\cs -> sLLAA $1 $> $ HsFunTy (ApiAnn (glAR $1) [mu AnnRarrow $2] cs) $1 $3) }

typedoc :: { LHsType GhcPs }
        : btype                          { $1 }
        | btype docprev                  { reLocA $ sLLAl $1 $> $ HsDocTy (ApiAnn (glAR $1) [] noCom) $1 $2 }
        | docnext btype                  { reLocA $ sLLlA $1 $> $ HsDocTy (ApiAnn (glR $1) [] noCom) $2 $1 }
        | btype '->'     ctypedoc        {% amsA $1 [mu AnnRarrow $2] -- See note [GADT decl discards annotations]
                                         >> acsA (\cs -> sLLAA $1 $> $ HsFunTy (ApiAnn (glAR $1) [mu AnnRarrow $2] cs) $1 $3) }
        | btype docprev '->' ctypedoc    {% amsA $1 [mu AnnRarrow $3] -- See note [GADT decl discards annotations]
                                         >> acsA (\cs -> sLLAA $1 $> $
                                                 HsFunTy (ApiAnn (glAR $1) [mu AnnRarrow $3] cs) (reLocA $ L (comb2 (reLoc $1) $2)
                                                            (HsDocTy noAnn $1 $2))
                                                         $4) }
        | docnext btype '->' ctypedoc    {% amsA $2 [mu AnnRarrow $3] -- See note [GADT decl discards annotations]
                                         >> acsA (\cs -> sLLlA $1 $> $
                                                 HsFunTy (ApiAnn (glR $1) [mu AnnRarrow $3] cs) (reLocA $ L (comb2 $1 (reLoc $2))
                                                            (HsDocTy noAnn $2 $1))
                                                         $4) }
...
-- See Note [Constr variations of non-terminals]
constr_tyapps :: { Located [LocatedA TyEl] } -- NB: This list is reversed
        : constr_tyapp                  { sL1A $1 [$1] }
        | constr_tyapps constr_tyapp    { sLLlA $1 $> $ $2 : (unLoc $1) }

-- See Note [Constr variations of non-terminals]
constr_tyapp :: { LocatedA TyEl }
        : tyapp                         { $1 }
        | docprev                       { sL1a $1 $ TyElDocPrev (unLoc $1) }

btype :: { LHsType GhcPs }
        : tyapps                        {% mergeOps $1 }

tyapps :: { [LocatedA TyEl] } -- NB: This list is reversed
        : tyapp                         { [$1] }
        | tyapps tyapp                  { $2 : $1 }
...
atype :: { LHsType GhcPs }
        ...
        | '{' fielddecls '}'             {% do { decls <- acsA (\cs -> (sLL $1 $> $ HsRecTy (ApiAnn (glR $1) [moc $1,mcc $3] cs) $2))
        ...
        | '[' ktype ']'               {% acsA (\cs -> sLL $1 $> $ HsListTy (ApiAnn (glR $1) (AnnParen AnnParensSquare (glR $1) (glR $3)) cs) $2) }
        | '(' ktype ')'               {% acsA (\cs -> sLL $1 $> $ HsParTy  (ApiAnn (glR $1) (AnnParen AnnParens       (glR $1) (glR $3)) cs) $2) }
...
tv_bndr :: { LHsTyVarBndr Specificity GhcPs }
        : tv_bndr_no_braces             { $1 }
        | '{' tyvar '}'                 {% acs (\cs -> sLL $1 $> (UserTyVar (ApiAnn (glR $1) [mop $1, mcp $3] cs) InferredSpec $2)) }
...
gadt_constrs :: { Located [LConDecl GhcPs] }
        : gadt_constr_with_doc ';' gadt_constrs
                  {% do { h <- addAnnotationA $1 AnnSemi (gl $2)
                        ; return (L (comb2 (reLoc $1) $3) (h : unLoc $3)) }}
        | gadt_constr_with_doc          { L (glA $1) [$1] }
        | {- empty -}                   { noLoc [] }
...
gadt_constr :: { LConDecl GhcPs }
    ...
    -- Returns a list because of:   C,D :: ty
        : con_list '::' sigtypedoc
                {% acsA (\cs -> sLLlA $1 $> $ mkGadtDecl (gl $1) cs (unLoc $1) $3 [mu AnnDcolon $2]) }
...
constrs1 :: { Located [LConDecl GhcPs] }
        : constrs1 maybe_docnext '|' maybe_docprev constr
            {% do { let (h:t) = addConDocFirst (unLoc $1) $4
                  ; h' <- addAnnotationA h AnnVbar (gl $3)
                  ; return (sLLlA $1 $> (addConDoc $5 $2 : h' : t)) }}
        | constr                                          { sL1A $1 [$1] }
...
constr :: { LConDecl GhcPs }
        : maybe_docnext forall constr_context '=>' constr_stuff
                {% acsa (\cs -> let (con,details,doc_prev) = unLoc $5 in
                  addConDoc (reLocA $ L (comb4 $2 (reLoc $3) $4 $5)
                                         (mkConDeclH98 (ApiAnn (glR $2) (mu AnnDarrow $4:(fst $ unLoc $2)) cs)
                                                       con
                                                       (snd $ unLoc $2)
                                                       (Just $3)
                                                       details))
        ...
        | maybe_docnext forall constr_stuff
                {% acsa (\cs -> let (con,details,doc_prev) = unLoc $3 in
                  addConDoc (reLocA $ L (comb2 $2 $3)
                                        (mkConDeclH98 (ApiAnn (glR $2) (fst $ unLoc $2) cs)
                                                      con
                                                      (snd $ unLoc $2)
                                                      Nothing   -- No context
                                                      details))
...
constr_stuff :: { Located (LocatedN RdrName, HsConDeclDetails GhcPs, Maybe LHsDocString) }
...
fielddecls1 :: { [LConDeclField GhcPs] }
        : fielddecl maybe_docnext ',' maybe_docprev fielddecls1
            {% do { h <- addAnnotationA (addFieldDoc $1 $4) AnnComma (gl $3)
                  ; return (h : addFieldDocs $5 $2) }}
        | fielddecl   { [$1] }
...
deriv_clause_types :: { LocatedC [LHsSigType GhcPs] }
        : qtycondoc           { sL1a (reLoc $1) [mkLHsSigType $1] }
        | '(' ')'             {% amsrc (sLL $1 $> [])
...
decl_no_th :: { LHsDecl GhcPs }
        ...
        | infixexp     opt_sig rhs  {% runECP_P $1 >>= \ $1 ->
                                       do { let { l = comb2Al $1 $> }
                                          ; r <- checkValDef l (fst $2 ++ (fst $ unLoc $3)) $1 (snd $2) $3;
                                        -- Depending upon what the pattern looks like we might get either
                                        -- a FunBind or PatBind back from checkValDef. See Note
                                        -- [FunBind vs PatBind]
                                        -- case r of {
                                        --   (FunBind _ n _ _) ->
                                        --         amsL l (mjA AnnFunId n:(fst $2)) >> return () ;
                                        --   (PatBind _ (L lh _lhs) _rhs _) ->
                                        --         amsL (locA lh) (fst $2) >> return () } ;
...
rhs     :: { Located ([AddApiAnn],GRHSs GhcPs (LHsExpr GhcPs)) }
        : '=' exp wherebinds    {% runECP_P $2 >>= \ $2 ->
                                  do { let loc = (comb3 $1 (reLoc $2) $3)
                                     ; return $
                                       sL loc ((mj AnnEqual $1 : (fst $ unLoc $3))
                                              , GRHSs noExtField (unguardedRHS (ApiAnn (rs loc) [] []) loc $2)
                                                      (snd $ unLoc $3)) } }
...
gdrh :: { LGRHS GhcPs (LHsExpr GhcPs) }
        : '|' guardquals '=' exp  {% runECP_P $4 >>= \ $4 ->
                                     acs (\cs -> sL (comb2A $1 $>) $ GRHS (ApiAnn (glR $1) [mj AnnVbar $1,mj AnnEqual $3] cs) (unLoc $2) $4) }
...
infixexp :: { ECP }
        ...
        | infixexp qop exp10p    -- See Note [Pragmas and operator fixity]
                               { ECP $
                                 ...
                                 runECP_PV $3 >>= \ $3 ->
                                 rejectPragmaPV $1 >>
                                 ammsA (mkHsOpAppPV (noAnnSrcSpan $ comb2A (reLoc $1) $>) $1 $2 $3
                                       (ApiAnn (glAR $1) [mjN AnnVal $2] noCom))
                                     [mjN AnnVal $2] }
...
exp10 :: { ECP }
        : '-' fexp                      { ECP $
                                           runECP_PV $2 >>= \ $2 ->
                                           mkHsNegAppPV (noAnnSrcSpan $ comb2A $1 $>) $2
                                                 [mj AnnMinus $1] }
        | fexp                         { $1 }
...
fexp    :: { ECP }
        ...
        -- See Note [Whitespace-sensitive operator parsing] in GHC.Parser.Lexer
-- AZ: fixup needed here
        -- | fexp PREFIX_AT atype       { ECP $
        --                                 runECP_PV $1 >>= \ $1 ->
        --                                 amms (mkHsAppTypePV (comb2 $1 $>) $1 $3) [mj AnnAt $2] }
        | fexp PREFIX_AT atype       { ECP $
                                        runECP_PV $1 >>= \ $1 ->
                                        mkHsAppTypePV (noAnnSrcSpan $ comb2 (reLoc $1) (reLoc $>)) $1 $3 [mj AnnAt $2] }

        | 'static' aexp              {% runECP_P $2 >>= \ $2 ->
                                        fmap ecpFromExp $
                                        fmap reLocA $ ams (\_ -> sLL $1 (reLoc $>) $ HsStatic noExtField $2)
                                            [mj AnnStatic $1] }
        | aexp                       { $1 }
...
aexp    :: { ECP }
        ...
        | PREFIX_TILDE aexp     { ECP $
                                   runECP_PV $2 >>= \ $2 ->
                                   mkHsLazyPatPV (comb2 $1 (reLoc $>)) (reLoc $2) [mj AnnTilde $1] }
        | PREFIX_BANG aexp      { ECP $
                                   runECP_PV $2 >>= \ $2 ->
                                   mkHsBangPatPV (comb2 $1 (reLoc $>)) (reLoc $2) [mj AnnBang $1] }

        | '\\' apat apats '->' exp
                   {  ECP $
                      runECP_PV $5 >>= \ $5 ->
                      mkHsLamPV (comb2 $1 (reLoc $>)) (mkMatchGroup FromSource
                            [sLLlA $1 $> $ Match { m_ext = noAnn
                                                 , m_ctxt = LambdaExpr
                                                 , m_pats = $2:$3
        ...
        | 'let' binds 'in' exp          {  ECP $
                                           runECP_PV $4 >>= \ $4 ->
                                           mkHsLetPV (comb2A $1 $>) (snd (unLoc $2)) $4
                                                 (mj AnnLet $1:mj AnnIn $3
                                                 :(fst $ unLoc $2)) }
        | '\\' 'lcase' altslist
            {  ECP $ $3 >>= \ $3 ->
                 mkHsLamCasePV (comb2 $1 $>)
                                   (mkMatchGroup FromSource (snd $ unLoc $3)) (mj AnnLam $1:mj AnnCase $2:(fst $ unLoc $3)) }
        ...
        | 'case' exp 'of' altslist    {% runECP_P $2 >>= \ $2 ->
                                         return $ ECP $
                                           $4 >>= \ $4 ->
                                           mkHsCasePV (comb3 $1 $3 $4) $2 (mkMatchGroup
                                                   FromSource (snd $ unLoc $4))
                                                (ApiAnnHsCase (gl $1) (gl $3) (fst $ unLoc $4)) }
        | 'do' stmtlist              { ECP $
                                        $2 >>= \ $2 ->
                                        mkHsDoPV (comb2 $1 $2) (mapLoc snd $2)
                                               (ApiAnn (glR $1) (mj AnnDo $1:(fst $ unLoc $2)) noCom)}
        | 'mdo' stmtlist            {% runPV $2 >>= \ $2 ->
                                       fmap ecpFromExp $
                                       acsA (\cs -> L (comb2 $1 $2)
                                              (mkHsDoAnns MDoExpr (snd $ unLoc $2)
                                           (ApiAnn (glR $1) (mj AnnMdo $1:(fst $ unLoc $2)) cs))) }
        | 'proc' aexp '->' exp
...
--                                       (getSTRING $1) noExtField) }
        ...
        | '(' texp ')'                  { ECP $
                                           runECP_PV $2 >>= \ $2 ->
                                           mkHsParPV (comb2 $1 $>) $2 [mop $1,mcp $3] }
        | '(' tup_exprs ')'             { ECP $
                                           $2 >>= \ $2 ->
                                           mkSumOrTuplePV (noAnnSrcSpan $ comb2 $1 $>) Boxed (snd $2)
                                                ((mop $1:fst $2) ++ [mcp $3])}
        ...
        | '(|' aexp cmdargs '|)'  {% runECP_P $2 >>= \ $2 ->
                                      fmap ecpFromCmd $
                                      acsA (\cs -> sLL $1 $> $ HsCmdArrForm (ApiAnn (glR $1) [mu AnnOpenB $1,mu AnnCloseB $4] cs) $2 Prefix
                                                           Nothing (reverse $3)) }
...
-- in GHC.Hs.Expr.
tup_exprs :: { forall b. DisambECP b => PV ([AddApiAnn],SumOrTuple b) }
           : texp commas_tup_tail
                           { runECP_PV $1 >>= \ $1 ->
                             $2 >>= \ $2 ->
                             do { t <- amsA (sL1 $1 (Just $1)) [AddApiAnn AnnComma (rs $ fst $2)]
           ...
           | commas tup_tail
                 { $2 >>= \ $2 ->
                   do { cos <- mapM (\ll -> amsr (L ll Nothing) [AddApiAnn AnnComma (rs ll)]) (fst $1)
                      ; return ([],Tuple (cos ++ $2)) } }
...
commas_tup_tail : commas tup_tail
        { $2 >>= \ $2 ->
          do { cos <- mapM (\l -> amsr (L l Nothing) [AddApiAnn AnnComma (rs l)]) (tail $ fst $1)
...
tup_tail :: { forall b. DisambECP b => PV [LocatedA (Maybe (LocatedA b))] }
          : texp commas_tup_tail { runECP_PV $1 >>= \ $1 ->
                                   $2 >>= \ $2 ->
                                   do { t <- amsA (L (gl $1) (Just $1)) [AddApiAnn AnnComma (rs $ fst $2)]
                                      ; return (t : snd $2) } }
          | texp                 { runECP_PV $1 >>= \ $1 ->
...
-- Never empty.
list :: { forall b. DisambECP b => SrcSpan -> [AddApiAnn] -> PV (LocatedA b) }
        : texp    { \loc ann -> runECP_PV $1 >>= \ $1 ->
                            mkHsExplicitListPV loc [$1] ann }
        | lexps   { \loc ann -> $1 >>= \ $1 ->
                            mkHsExplicitListPV loc (reverse $1) ann }
        | texp '..'  { \loc ann -> runECP_PV $1 >>= \ $1 ->
                                  acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [mj AnnDotdot $2] cs) Nothing (From $1))
                                      >>= ecpFromExp' }
        | texp ',' exp '..' { \loc ann ->
                                   runECP_PV $1 >>= \ $1 ->
                                   runECP_PV $3 >>= \ $3 ->
                                   acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [mj AnnComma $2,mj AnnDotdot $4] cs) Nothing (FromThen $1 $3))
                                       >>= ecpFromExp' }
        | texp '..' exp  { \loc ann ->
                                   runECP_PV $1 >>= \ $1 ->
                                   runECP_PV $3 >>= \ $3 ->
                                   acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [mj AnnDotdot $2] cs) Nothing (FromTo $1 $3))
                                       >>= ecpFromExp' }
        | texp ',' exp '..' exp { \loc ann ->
                                   runECP_PV $1 >>= \ $1 ->
                                   runECP_PV $3 >>= \ $3 ->
                                   runECP_PV $5 >>= \ $5 ->
                                   acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [mj AnnComma $2,mj AnnDotdot $4] cs) Nothing (FromThenTo $1 $3 $5))
                                       >>= ecpFromExp' }
        | texp '|' flattenedpquals
             { \loc ann ->
                checkMonadComp >>= \ ctxt ->
                runECP_PV $1 >>= \ $1 ->
                acsA (\cs -> L loc $ mkHsCompAnns ctxt (unLoc $3) $1 (ApiAnn (realSrcSpan loc) [mj AnnVbar $2] cs))
                    >>= ecpFromExp' }
...
transformqual :: { Located ([LStmt GhcPs (LHsExpr GhcPs)] -> Stmt GhcPs (LHsExpr GhcPs)) }
    ...
    : 'then' exp              {% runECP_P $2 >>= \ $2 ->
                                 acs (\cs->
                                 sLLlA $1 $> (\ss -> (mkTransformStmt (ApiAnn (glR $1) [mj AnnThen $1] cs) ss $2))) }
    | 'then' exp 'by' exp     {% runECP_P $2 >>= \ $2 ->
                                 runECP_P $4 >>= \ $4 ->
                                 acs (\cs -> sLLlA $1 $> (
                                                     \ss -> (mkTransformByStmt (ApiAnn (glR $1) [mj AnnThen $1,mj AnnBy $3] cs) ss $2 $4))) }
    | 'then' 'group' 'using' exp
            {% runECP_P $4 >>= \ $4 ->
               acs (\cs -> sLLlA $1 $> (
                                   \ss -> (mkGroupUsingStmt (ApiAnn (glR $1) [mj AnnThen $1,mj AnnGroup $2,mj AnnUsing $3] cs) ss $4))) }

    | 'then' 'group' 'by' exp 'using' exp
            {% runECP_P $4 >>= \ $4 ->
               runECP_P $6 >>= \ $6 ->
               acs (\cs -> sLLlA $1 $> (
                                   \ss -> (mkGroupByUsingStmt (ApiAnn (glR $1) [mj AnnThen $1,mj AnnGroup $2,mj AnnBy $3,mj AnnUsing $5] cs) ss $4 $6))) }
...
altslist :: { forall b. DisambECP b => PV (Located ([AddApiAnn],[LMatch GhcPs (LocatedA b)])) }
        : '{'            alts '}'  { $2 >>= \ $2 -> return $
...
alts1   :: { forall b. DisambECP b => PV (Located ([AddApiAnn],[LMatch GhcPs (LocatedA b)])) }
        : alts1 ';' alt         { $1 >>= \ $1 ->
                                  $3 >>= \ $3 ->
                                     ...
                                     then return (sLL $1 $> ((mz AnnSemi $2) ++(fst $ unLoc $1)
                                                  ,[$3]))
                                     else (ams (\_ -> head $ snd $ unLoc $1)
                                               ((mz AnnSemi $2) ++ (fst $ unLoc $1))
...
alt     :: { forall b. DisambECP b => PV (LMatch GhcPs (LocatedA b)) }
           : pat alt_rhs  { $2 >>= \ $2 ->
                            acs (\cs -> sLL (reLoc $1) $> (Match { m_ext = (ApiAnn (glAR $1) (fst $ unLoc $2) cs)
                                                  , m_ctxt = CaseAlt
                                                  , m_pats = [$1]
                                                  , m_grhss = snd $ unLoc $2 }))}

alt_rhs :: { forall b. DisambECP b => PV (Located ([AddApiAnn],GRHSs GhcPs (LocatedA b))) }
...
ralt :: { forall b. DisambECP b => PV (Located [LGRHS GhcPs (LocatedA b)]) }
        : '->' exp            { runECP_PV $2 >>= \ $2 ->
                                acs (\cs -> sLLlA $1 $> (unguardedRHS (ApiAnn (glR $1) [mu AnnRarrow $1] cs) (comb2 $1 (reLoc $2)) $2)) }
        | gdpats              { $1 >>= \gdpats ->
...
gdpat   :: { forall b. DisambECP b => PV (LGRHS GhcPs (LocatedA b)) }
        : '|' guardquals '->' exp
                                   { runECP_PV $4 >>= \ $4 ->
                                     acs (\cs -> sL (comb2A $1 $>) $ GRHS (ApiAnn (glR $1) [mj AnnVbar $1,mu AnnRarrow $3] cs) (unLoc $2) $4) }
...
stmtlist :: { forall b. DisambECP b => PV (Located ([AddApiAnn],[LStmt GhcPs (LocatedA b)])) }
        : '{'           stmts '}'       { $2 >>= \ $2 -> return $
                                          sLL $1 $> ((moc $1:mcc $3:(fst $ unLoc $2))
...
stmts :: { forall b. DisambECP b => PV (Located ([AddApiAnn],[LStmt GhcPs (LocatedA b)])) }
        : stmts ';' stmt  { $1 >>= \ $1 ->
                            $3 >>= \ $3 ->
                            if null (snd $ unLoc $1)
                              then return (sLL $1 (reLoc $>) ((mz AnnSemi $2) ++ (fst $ unLoc $1)
                                                     ,$3   : (snd $ unLoc $1)))
                              else do
                               { amsA (head $ snd $ unLoc $1) (mz AnnSemi $2)
                               ; return $ sLL $1 (reLoc $>) (fst $ unLoc $1,$3 :(snd $ unLoc $1)) }}

        | stmts ';'     {  $1 >>= \ $1 ->
                           if null (snd $ unLoc $1)
                             then return (sLL $1 $> ((mz AnnSemi $2) ++ (fst $ unLoc $1),snd $ unLoc $1))
                             else do
                               { amsA (head $ snd $ unLoc $1) (mz AnnSemi $2)
                               ; return $1 }
          }
...
qual  :: { forall b. DisambECP b => PV (LStmt GhcPs (LocatedA b)) }
    : bindpat '<-' exp                   { runECP_PV $3 >>= \ $3 ->
                                           amsr (sLLlA (reLoc $1) $> $ mkPsBindStmt $1 $3)
                                               [mu AnnLarrow $2] }
    | exp                                { runECP_PV $1 >>= \ $1 ->
...
dbinds  :: { Located [LIPBind GhcPs] } -- reversed
        : dbinds ';' dbind
                      -- {% addAnnotationS (gl $ last $ unLoc $1) AnnSemi (gl $2) >>
...
gen_qcon :: { LocatedN RdrName }
  ...
  | '(' qconsym ')'       {% amsrn (sLL $1 $> (unLoc $2))
                                   (NameAnn NameParens (glR $1) (glNR $2) (glR $3) []) }
...
sysdcon_nolist :: { LocatedN DataCon }  -- Wired in data constructors
        : '(' ')'               {% amsrn (sLL $1 $> unitDataCon) (NameAnnOnly NameParens (glR $1) (glR $2) []) }
        | '(' commas ')'        {% amsrn (sLL $1 $> $ tupleDataCon Boxed (snd $2 + 1))
                                       (NameAnnCommas NameParens (glR $1) (map realSrcSpan (fst $2)) (glR $3) []) }
        | '(#' '#)'             {% amsrn (sLL $1 $> $ unboxedUnitDataCon) (NameAnnOnly NameParensHash (glR $1) (glR $2) []) }
        | '(#' commas '#)'      {% amsrn (sLL $1 $> $ tupleDataCon Unboxed (snd $2 + 1))
                                       (NameAnnCommas NameParensHash (glR $1) (map realSrcSpan (fst $2)) (glR $3) []) }

-- See Note [Empty lists] in GHC.Hs.Expr
sysdcon :: { LocatedN DataCon }
        : sysdcon_nolist                 { $1 }
        | '[' ']'               {% amsrn (sLL $1 $> nilDataCon) (NameAnnOnly NameSquare (glR $1) (glR $2) []) }

conop :: { LocatedN RdrName }
        : consym                { $1 }
        | '`' conid '`'         {% amsrn (sLL $1 $> (unLoc $2))
                                           (NameAnn NameBackquotes (glR $1) (glNR $2) (glR $3) []) }
...
ntgtycon :: { LocatedN RdrName }  -- A "general" qualified tycon, excluding unit tuples
        ...
        | '(' '->' ')'          {% amsrn (sLL $1 $> $ getRdrName funTyCon)
                                          (NameAnn NameParens (glR $1) (glR $2) (glR $3) []) }
        | '[' ']'               {% amsrn (sLL $1 $> $ listTyCon_RDR) (NameAnnOnly NameSquare (glR $1) (glR $2) []) }
...
oqtycon_no_varcon :: { LocatedN RdrName }  -- Type constructor which cannot be mistaken
        ...
        | '(' QCONSYM ')'    {% let { name :: Located RdrName
                                ...
                                in amsrn (sLL $1 $> (unLoc name)) (NameAnn NameParens (glR $1) (glR $2) (glR $3) []) }
...
mj a l = AddApiAnn a (rs $ gl l)

mjA :: AnnKeywordId -> LocatedAn t e -> AddApiAnn
...
mz a l = if isZeroWidthSpan (gl l) then [] else [AddApiAnn a (rs $ gl l)]

-- |Construct an ApiAnn from the annotation keyword and the location
-- of the keyword itself
ma :: AnnKeywordId -> Located e -> ApiAnnComments -> ApiAnn
ma a l cs = ApiAnn (glR l) [mj a l] cs
...
glAR :: LocatedAn t a -> RealSrcSpan
...
aa a@(L l _) (b,s) = addAnnotationS l b (gl s) >> return a

-- |Add an annotation to a located element resulting from a monadic action
am :: P (Located a) -> (AnnKeywordId, Located b) -> P (Located a)
am a (b,s) = do
  av@(L l _) <- a
  addAnnotationS l b (gl s)
  return av

-- | Add a list of AddApiAnns to the given AST element.  For example,
-- the parsing rule for @let@ looks like:
--
-- @
--      | 'let' binds 'in' exp    {% ams (\_ -> sLL $1 $> $ HsLet (snd $ unLoc $2) $4)
--                                       (mj AnnLet $1:mj AnnIn $3
--                                         :(fst $ unLoc $2)) }
-- @
--
...
amsA (L l a) bs = do
  ...
  let aa = addAnnsA (ann l) bs cs
  return (L (SrcSpanAnn aa (locA l)) a)

-- AZ:TODO: this has to go, keep for now as temporary measure
...
-- AZ:TODO: this has to go, keep for now as temporary measure
reN :: MonadP m => Located a -> LocatedN b -> [AddApiAnn] -> m (LocatedN b)
reN x y@(L la b) bs = do
  let l = comb2N x y
  amsN (L (noAnnSrcSpan l) b) bs
...
amsrn (L l a) an = do
  cs <- addAnnsAt l []
  -- AZ: TODO actually use this, next step
  let ann = (ApiAnn (realSrcSpan l) an cs)
  return (L (SrcSpanAnn ann l) a)

amsL :: SrcSpan -> [AddApiAnn] -> P ()
amsL sp bs = addAnnsAt sp bs >> return ()

-- |Add all [AddApiAnn] to an AST element, and wrap it in a 'Just'
...
-- | Add a list of AddApiAnns to the given AST element, where the AST
--  element is the result of a monadic action
ammsA :: MonadP m => m (LocatedAn t a) -> [AddApiAnn] -> m (LocatedAn t a)
ammsA a bs = do { av@(L l _) <- a
                ; addAnnsAt (locA l) bs
                ; return av }

-- | Add a list of AddApiAnns to the given AST element, where the AST
--  element is the result of a monadic action
ammsA' :: MonadP m => m (Located a) -> [AddApiAnn] -> m (LocatedAn t a)
ammsA' a bs = do { av@(L l _) <- a
                 ; cs <- addAnnsAt l bs
                 ; return (reLocA av) }


-- |Add a list of AddApiAnns to the AST element, and return the element as a
--  OrdList
amsu :: (ApiAnnComments -> Located a) -> [AddApiAnn] -> P (OrdList (Located a))
amsu a bs = do
  let (L l _) = a []
  cs <- addAnnsAt l bs
  return (unitOL (a cs))
...
-- |Given a list of the locations of commas, provide a [AddApiAnn]
--  with an AnnComma entry for each SrcSpan
mcommas :: [SrcSpan] -> [AddApiAnn]
mcommas = map (AddApiAnn AnnCommaTuple . rs)

-- |Given a list of the locations of '|'s, provide a [AddApiAnn] with an AnnVbar
--  entry for each SrcSpan
mvbars :: [SrcSpan] -> [AddApiAnn]
mvbars = map (AddApiAnn AnnVbar . rs)
...
ollA l =
  if isNilOL l then noSrcSpan
               else getLocA (lastOL l)


-- |Add a semicolon annotation in the right place in a list. If the
-- leading list is empty, add it to the tail
asl :: [Located a] -> Located b -> Located a -> P ()
asl [] (L ls _) (L l _) = addAnnotationS l          AnnSemi ls
asl (x:_xs) (L ls _) _x = addAnnotationS (getLoc x) AnnSemi ls
...
addAnnotationS :: MonadP m => SrcSpan -- SrcSpan of enclosing AST construct
               ...
               -> m ()
addAnnotationS (RealSrcSpan l _) kw s@(RealSrcSpan v _)
  = if isZeroWidthSpan s then return () else addAnnotation l kw v
addAnnotationS _ _ _ = return ()
...
addAnnotationA (L la a) kw span = do
  ...
  let anns' = if isZeroWidthSpan span
                then addAnnsA (ann la) [] cs
                else addAnnsA (ann la) [AddApiAnn kw (rs span)] cs
...
addTrailingCommaN (L (SrcSpanAnn anns l) a) span = do
  ...
  let anns' = if isZeroWidthSpan span
                then anns
                else addTrailingComma l anns (rs span)
